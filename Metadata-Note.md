...
### Ch3 描述性元数据
> P067
- 标准就好像是牙刷，每个人都觉得刷牙是个好主意，但是谁也不愿意用别人的牙刷。

> P073 Dublin Core
- [15个核心元素](http://dublincore.org/documents/dces/)
  - Contributor
  - Coverage
  - Creator
  - Date
...
> P095 技术性元数据
- 不需要人为判断即可识别的数据，可以自动收集

> P098 结构性元数据
- 记录关于某一资源如何组织的信息

> P099 溯源元数据
- 了解在线资源的历史很有必要，但是却并不充分。为了可以信任资源的有效性与可靠性，同样有必要了解什么实体影响过资源的历史。
- ...谁在做出陈述...将资源至于社交网络之中的方法，借此提供用户评估资源可能需要的“上下文”...了解资源是否可信。
- W3C...溯源数据模型...三大核心结构为实体（entity，资源）、代理（agent，影响资源生命周期的实体）和活动（activity，这种影响的本质）

> P103 保存性元数据
- PREMIS，搜集关于如何在一段时间内保存数字对象所必需的最小元素集
- 保存过程需要的信息
  - 持久生存能力 viability
  - 可呈现能力 renderability
...
> P105 权限元数据
- 授权（license）
- 权限持有者（rightsHolder，个人或组织）
- 使用权（accessRights，权限持有者根据授权中理应使用资源所必须拥有的权限）

> P108 meta-metadata
- METS，元数据编码与传输标准，管理一切的元数据模式
- 提供一种标准架构并且确保存储库之间可以交换元数据...支持为元数据记录创建容器

### Ch5 使用性元数据
> P122
- 数据废气，一个人进行日常活动所产生的数据`--大数据`

### Ch6 实现元数据的技术
> P134 RDF 资源描述框架
- RDF是用于描述资源的框架...一种数据模型...一种对实体做出描述性陈述的通用数据模型
- 三段式主谓宾关系（三元组）是RDF的核心部分
- 三元组的主题必须采用URL进行标识，才能在三元组中或由在线服务中加以引用

> P138
- 从哲学的角度来说，这是本体论的基本问题：当各方无法就宇宙中相同的实体类别达成共识时...就会让沟通变成一种挑战。`先定义，再调用；新人入职时，需要先培训基于行业、公司的基本概念，才能培训业务`
- 被描述资源由属性-值配对进行描述
- 任何实体都能被描述，都可以成为RDF三元组的主题

> P139 XML
- 在实践中，许多元数据模式的语法和语义均采用XML为编码格式
- DOM，文档对象模型，所有近代浏览器都具有引用这种DOM的功能，并且可以据此解读HTML文档中使用的元素。

### Ch7 语义网 Semantic Web0
> P150
- 为了让语义网的愿景成为现实，就要依靠软件代理，运用元数据对有意义的网络内容施加结构
- ...不仅仅是将结构化数据上传到网上，还需要在结构化数据之间建立联系

> P152
- 语义网要求一个资源要拥有一个URI，这样其它资源或在线服务就能明确无误地引用这个资源
- 可在某个URI访问的资源被称为“可解引用”...HTTP是引用URI的首选机制...
- 关联数据的愿景是，任何类型都可以通过软件来解引用，而向查询代理返回的不仅仅是资源，而且还有关于该资源的元数据
- 在对某个资源进行解引用时，提供给查询应用程序的除了资源本身，还有关于该资源所存在的任何元数据记录。该元数据中包括一个其他资源列表，原有链接会与其进行关联，即**该资源为主语的任何三元组的宾语，而任何其他资源为原有资源作为主语的三元组主语的宾语**。

> P154
- 如果资源可以解引用，就能在资源之间建立三元组网络。在这个三元组网络中，关联数据得以关联，由此也就为数据网络赋予了结构，而这也让软件代理得以运用这种结构来实施任务

> P165
- 有了“与...相同”元素，实体间关系网可以从维基百科现有知识的边界拓展到人类知识的边界

> P166 关联开放数据
- ...某个组织在网上公布一个数据集，从而得以运用该数据集之中以及该数据集内外实体之间的RDF三元组结构。当然，各个组织可以运用专有数据集中的RDF三元组并且利用其它组织的工作成果。
- ...运用schema.org...元数据进行标记：
  - <div itemscope itemtype= "http://schema.org/book" >...
  - ...itemtype声明本节是关于什么类型的项目，并表明Schema.org中该类型的声明在所提供的URL中。现在，任何解析这个网页的应用程序（也可以解读Schema.org元数据）可以解读“书”（Book）这项类型的属性，因为该URL已经枚举出了这些数据。Schema.org为“书”这项类型声明了许多属性。
  - Schema.org包含两部分：实体及其属性的集合，以及在网页中嵌入关于这些实体的数据所需要的语法。当这种结构数据嵌入在网页中时，...搜索工具可以利用这种数据让用户创建高度个性化以及经过筛选的搜索
  
### Ch8 元数据的未来
> P184
- API是网络上最引人注意的元数据用例之一...可以同时实现对资源以及元数据的访问

> P187
- 来回传递结构化数据能让在线服务得以保持较小的体积并且具有专注性，但是需要依赖其它服务来提供所需的数据。
